...
rsFloat Gaussian::GetGain(const rs::SVec3& angle, const rs::SVec3& refangle, rsFloat wavelength) const
{
	...
	const rsFloat azfactor = std::exp(-a.azimuth * a.azimuth * azscale);
	const rsFloat elfactor = std::exp(-a.elevation * a.elevation * elscale);
...
rsFloat Sinc::GetGain(const SVec3& angle, const SVec3& refangle, rsFloat wavelength) const
{
	...
	const rs::rsComplex complexSinc(::sinc(beta * theta), 0.0);
	const rs::rsComplex complexGamma(gamma, 0.0);

	//See "Sinc Pattern" in doc/equations.tex for equation used here
	const rsComplex complexGain = alpha * std::pow(complexSinc, complexGamma) * GetEfficiencyFactor();
...
{
	const rsFloat Ge = 4 * M_PI * dimension * dimension / (wavelength * wavelength);
	const rsFloat x = M_PI * dimension * std::sin(GetAngle(angle, refangle)) / wavelength;
...
{
	const rsFloat Ge = std::pow(M_PI * diameter / wavelength, 2);
	const rsFloat x = M_PI * diameter * std::sin(GetAngle(angle, refangle)) / wavelength;
...
rsFloat XMLAntenna::GetGain(const SVec3& angle, const SVec3& refangle, rsFloat wavelength) const
{
	...
	const rsFloat azi_gain = azi_samples->Value(std::fabs(t_angle.azimuth));
	const rsFloat elev_gain = elev_samples->Value(std::fabs(t_angle.elevation));
...
rsFloat PythonAntenna::GetGain(const SVec3& angle, const SVec3& refangle, rsFloat wavelength) const
{
	...
	const rsFloat gain = py_antenna.GetGain(angle_bore);
	return gain * GetEfficiencyFactor();
}
