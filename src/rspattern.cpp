...
Pattern::Pattern(const std::string& filename)
{
	...
	_pattern = rshdf5::readPattern(filename, "antenna", _size_azi, _size_elev);
}
...
{
	//Clean up
	for (unsigned int i = 0; i < _size_azi; i++)
	{
		delete[] _pattern[i];
	}
	delete[] _pattern;
...
rsFloat Pattern::getGain(const rs::SVec3& angle) const
{
	...
	const double y1 = std::floor((angle.elevation + M_PI) / (2 * M_PI) * (_size_elev - 1)) / static_cast<double>(_size_elev - 1);
	const double ey1 = (angle.elevation + M_PI) / (2 * M_PI);
	const double y2 = y1 + 1.0 / static_cast<double>(_size_elev);
