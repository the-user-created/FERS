...
namespace
{
	...
	void SolveRE(const Transmitter* trans, const Receiver* recv, const Target* targ, rsFloat time, rsFloat length,
	             RadarSignal* wave, REResults& results)
	{
	                    ...
	                    const TransmitterPulse* signal)
	{
		...
		try
		{
			...
			InterpPoint point(results.power, end_time + results.delay, results.delay, results.doppler, results.phase,
			                  results.noise_temperature);
			response->AddInterpPoint(point);
		}
		catch (RangeError& re)
	...
	void SolveREDirect(const Transmitter* trans, const Receiver* recv, rsFloat time, rsFloat length,
	...
	void AddDirect(const Transmitter* trans, Receiver* recv, const World* world, const TransmitterPulse* signal)
	{
		...
		try
		{
			...
			InterpPoint point(results.power, results.delay + end_time, results.delay, results.doppler, results.phase,
			                  results.noise_temperature);
			response->AddInterpPoint(point);
		}
		catch (RangeError& re)
